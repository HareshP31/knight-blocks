...
const GAZE_X_OFFSET_MIN = -0.015;
const GAZE_X_OFFSET_MAX = 0.015;
...
/**
 * Creates a FaceLandmarker instance from a given model path.
 * This is a helper function used by the main loading logic.
 */
...
/**
 * Main function exported to lab-main.js.
 * Loads models progressively: lightweight first, then heavy.
 */
...
/**
 * Loads the full iris model (17.6MB) in the background.
 */
async function loadIrisModel() {
  console.log("Loading AI Model (Precision - Gaze) in background...");
  try {
    // Add '?v=...' for aggressive cache-busting during development/hackathon
    const modelPath = 'https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker_with_blendshapes/float16/1/face_landmarker_with_blendshapes.task?v=hackathon_final';
    irisModel = await createLandmarker(modelPath);
    console.log("AI Model (Precision Gaze) loaded successfully.");
    window.dispatchEvent(new CustomEvent("gaze_ready")); // Fire event when ready
  
  } catch (error) {
    console.error("Error loading precision iris model (Gaze tracking disabled):", error);
